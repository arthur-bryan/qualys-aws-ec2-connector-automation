...
class AWSConnectorHanlder:
    ...
    def handle_connector_deletion_xml(name):
        ...
        delete_connector_xml = open(deletion_xml_file_name)
        tree = ET.ElementTree(ET.fromstring(delete_connector_xml.read()))
        root = tree.getroot()
        root.find(".//Criteria").text = name
        tree.write(deletion_xml_file_name)
        return open(deletion_xml_file_name).read()
